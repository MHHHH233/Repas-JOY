...
import endpoints from '../../endpoints/user/commandes';

const ok = (r) => (r.status === 200 || r.status === 201) ? r.data : Promise.reject(r);

const userCommandesService = {
    ...
    byUser: (userId, params = {}) => apiClient.get(endpoints.byUser(userId), { params }).then(ok),
};

export default userCommandesService;
